...
__interrupt void Timer_A(void) {		// define our Timer A's interrupt handler
    int i;
...
__interrupt void USCIAB0RX_ISR(void) {	// define our UART interrupt handler
    if(IFG2 && UCA0RXBUF) {
        UCA0TXBUF = UCA0RXBUF;			// echo back the message
    }
}
